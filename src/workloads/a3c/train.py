...
def train(rank, args, shared_model, optimizer, num_episode_trained, env_conf):
    ...
    try:
        while num_episode_trained < args.episodes:
            # see if num_episodes_trained is greater than the max number of episodes
            # num_episodes_trained is a tensor, while args.max_episode_length is an int
            if gpu_id >= 0:
                with torch.cuda.device(gpu_id):
            ...
            num_episode_trained += 1
            # print(f"train agent {rank} episode {num_episode_trained}", flush=True)
