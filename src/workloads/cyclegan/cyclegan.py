...
print(f"env available gpu {os.environ['CUDA_VISIBLE_DEVICES']}")
...
start_epoch = batches_done // len(dataloader)
...
for epoch in range(start_epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        if len(dataloader) * epoch + i <= batches_done:
            continue
        ...
        if i % 1 == 0:
            sys.stdout.write(
                "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, adv: %f, cycle: %f, identity: %f] Time passed: %s ETA: %s steps: %d"
                % (
                    epoch+1,
                    opt.n_epochs,
                    i,
        ...
        if batches_done % opt.checkpoint_interval == 0:
            # Save checkpoint including model and optimizer states
            state = {
