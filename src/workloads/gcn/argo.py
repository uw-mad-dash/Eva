...
class ARGO:
    def __init__(
        ...
        space=[(2, 8), (1, 4), (1, 32)],
        random_state=1,
        counter=0 # the current epoch to run
    ):
        """
        Initialization
        ...
        """
        self.n_search = min(1, epoch - counter)
    ...
    def core_binder(
        ...
        # map to actual core ids
        pid = os.getpid()
        cpu_ids = os.sched_getaffinity(pid)
        load_core = [list(cpu_ids)[i] for i in load_core]
        comp_core = [list(cpu_ids)[i] for i in comp_core]
    ...
    def auto_tuning(self, train: Callable, args) -> List[int]:
        ...
        result = gp_minimize(
            ...
            n_initial_points=min(10, self.n_search),
        )
        return result
    ...
    def mp_engine(self, x: List[int], train: Callable, args, ep: int) -> float:
        ...
        for i in range(n_proc):
            ...
            print(f"Process {i}: Load Cores: {load_core}, Compute Cores: {comp_core}")
            p = dmp.Process(
                target=train,
    ...
    def run(self, train, args):
        ...
        # x = result.x  # Step 2
        x = [100, 100, 100] # mp_engine will decide the actual configuration
        print("Optimal Configuration: ", x)
        print("counter is ", self.counter[0])
        print("epoch is ", self.epoch)
        if self.counter[0] == self.epoch:
            return
        self.mp_engine(
            x, train, args, ep=(self.epoch - self.counter[0])
        )  # Step 3
