...
def test(args, shared_model, num_episodes_trained, env_conf):
    ptitle("Test Agent")
    ...
    max_score = 0
    try:
        while num_episodes_trained < args.episodes:
            if player.done:
                if gpu_id >= 0:
                    with torch.cuda.device(gpu_id):
            ...
            if player.done and not player.env.was_real_done:
                ...
                log.info(
                    f'Time {time.strftime("%Hh %Mm %Ss", time.gmtime(time.time() - start_time))}, episode reward {reward_sum}, episode length {player.eps_len}, reward mean {reward_mean:.4f}, trained episodes {num_episodes_trained.item()}'
                )
                if args.tensorboard_logger:
                    writer.add_scalar(
                ...
                if (args.save_max and reward_sum >= max_score) or not args.save_max:
                    ...
                    if gpu_id >= 0:
                        with torch.cuda.device(gpu_id):
                            ...
                            state_to_save["num_episodes_trained"] = num_episodes_trained
                            torch.save(
                                state_to_save, f"{args.save_model_dir}{args.env}_best.dat"
                            )
                    else:
                        state_to_save = player.model.state_dict()
                        state_to_save["num_episodes_trained"] = num_episodes_trained
                ...
                if gpu_id >= 0:
                    with torch.cuda.device(gpu_id):
                        ...
                        state_to_save["num_episodes_trained"] = num_episodes_trained
                        torch.save(
                            state_to_save, f"{args.save_model_dir}{args.env}_last.dat"
                        )
                else:
                    state_to_save = player.model.state_dict()
                    state_to_save["num_episodes_trained"] = num_episodes_trained
